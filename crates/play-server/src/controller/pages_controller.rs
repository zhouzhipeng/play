...
use crate::{
    hex_to_string, method_router, promise, render_fragment, render_page, template, AppError,
};
use crate::{HTML, R, S};
...
async fn page_versions(s: S, Query(q): Query<QueryPageVersion>) -> HTML {
    ...
    promise!(
        rows.len() == 1 && rows[0].cat == "pages",
        "invalid data id , not pages"
    );
    ...
    for log in logs {
        ...
        handlers.push(tokio::spawn(async move {
            let before = if !log.data_before.is_empty() {
                hex_to_string!(
                    serde_json::from_str::<PageDto>(&log.data_before)
                        .unwrap()
                        .content
                )
            } else {
            ...
            let after = if !log.data_after.is_empty() {
                hex_to_string!(
                    serde_json::from_str::<PageDto>(&log.data_after)
                        .unwrap()
                        .content
                )
            } else {
            ...
            .await
            .unwrap_or(Html("server error".to_string()))
            .0;
...
async fn dynamic_pages(
    ...
    let data =
        GeneralData::query_by_json_field("*", "pages", "url", &format!("/{}", url), 1, &s.db)
            .await?;
...
mod tests {
    ...
    async fn test_text_compare() -> anyhow::Result<()> {
        ...
        html_output
            .push_str("table { width: 100%; table-layout: fixed; border-collapse: collapse; }");
