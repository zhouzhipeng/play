...
use crate::config::{AuthConfig, DomainProxy, OriginStrategy, ProxyTarget, WebSocketConfig};
...
pub async fn http_middleware(
    ...
    let prefix_log = format!(
        "served request >> method: {} , url :{}",
        request.method(),
        uri
    );
    ...
    if auth_config.enabled {
        let is_whitelist =
            uri == "/" || { auth_config.whitelist.iter().any(|x| uri.starts_with(x)) };
        ...
        if !is_whitelist {
            ...
            if f.is_empty() {
                ...
                if !auth_config.fingerprints.contains(&f) {
                    warn!(
                        "fingerprint not match for : {}, refuse to visit  uri : {}",
                        f, uri
                    );
                    //refuse
                    return refuse_response();
...
impl<B: Send + 'static> Service<Request<B>> for NotFoundService {
    ...
    fn call(&mut self, req: Request<B>) -> Self::Future {
        Box::pin(async move { Ok(handle_404(req.uri().clone()).await.into_response()) })
    }
}

pub async fn serve_domain_folder(
    state: S,
    host: String,
    request: Request<axum::body::Body>,
    folder_path: &str,
    ...
    #[cfg(feature = "play-dylib-loader")]
    {
        ...
        let plugin = state
            ...
            .plugin_config
            .iter()
            .find(|plugin| !plugin.disable && plugin.proxy_domain.eq(&host));
        if let Some(plugin) = plugin {
            return Ok(inner_run_plugin(plugin, request)
                .await
                .map_err(|e| anyhow!("{:?}", e))?);
        }
    }
...
pub async fn serve_upstream_proxy_with_config(
    ...
    if is_websocket {
        handle_websocket_origin(
            &mut request,
            &host,
            &scheme,
            ip,
            port,
            &domain_config.websocket_config,
        )?;
    }
    ...
    let mut proxy = if domain_config.ignore_cert && scheme == "https" {
        warn!(
            "Creating proxy with certificate verification DISABLED for {}",
            target_base
        );
        match create_ignore_cert_client() {
            Ok(custom_client) => ReverseProxy::new_with_client("", &target_base, custom_client)
                .with_ignore_cert(true),
            Err(e) => {
                warn!(
                    "Failed to create ignore cert client: {}, using standard proxy",
                    e
                );
        ...
        Ok(response) => {
            ...
            if is_websocket {
                ...
                if status == StatusCode::NOT_FOUND {
                    warn!(
                        "404 response for path: {} (target: {}{})",
                        path_and_query, target_base, path_and_query
                    );
                } else if status.is_success() {
                    info!("Success: {} for path: {}", status, path_and_query);
                } else {
                    warn!(
                        "Non-success status: {} for path: {}",
...
fn handle_websocket_origin(
    ...
    info!(
        "Handling WebSocket Origin with strategy: {:?}",
        websocket_config.origin_strategy
    );
    ...
    match &websocket_config.origin_strategy {
        ...
        OriginStrategy::Custom => {
            ...
            if let Some(custom_origin) = &websocket_config.custom_origin {
                ...
                info!(
                    "Set WebSocket Origin to backend (fallback): {}",
                    backend_origin
                );
            }
        }
...
async fn handle_custom_proxy_with_ignore_cert(
    ...
    if is_websocket {
        info!(
            "WebSocket upgrade request with ignored certificates: {}",
            target_uri
        );
        ...
        match target_uri.parse::<hyper::Uri>() {
            ...
            Err(e) => {
                warn!(
                    "Failed to parse target URI for WebSocket: {}, error: {}",
                    target_uri, e
                );
        ...
        match create_ignore_cert_client() {
            Ok(custom_client) => {
                info!(
                    "WebSocket: Using custom client with certificate verification DISABLED for {}",
                    target_base
                );
                let mut proxy = axum_reverse_proxy::ReverseProxy::new_with_client(
                    "",
                    &target_base,
                    custom_client,
                )
    ...
    // 构建reqwest请求
    let mut req_builder = client.request(method, target_uri).body(body_bytes.to_vec());
    ...
    Ok(response_builder.body(Body::from(response_bytes.to_vec()))?)
}

// 创建忽略证书验证的HTTP客户端
fn create_ignore_cert_client() -> Result<
